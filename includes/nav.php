 <header id="home">
   <div class="border-bottom pre-header align-items-center ">
     <div class="container-lg">
       <div class="d-flex flex-column flex-md-row justify-content-md-between align-items-center">
         <!-- Información de contacto -->
         <ul class="list-unstyled d-flex gap-3 mb-2 mb-md-0">
           <li class="telefono">
             <a href="tel:+346666666" class="text-decoration-none"> +34 666 66 66</a>
           </li>
           <li>
             <a href="mailto:correo@correo" target="_blank" class="text-decoration-none">correo@correo</a>
           </li>
         </ul>
         <!-- Redes sociales -->
        <ul class="d-flex justify-content-center align-items-center gap-2 list-unstyled border-top border-md-0 pt-1 pt-md-2 pb-1 pb-md-1 mb-1 mb-md-0">


           <li>
             <a href="<?php echo $base; ?>/quienes-somos.php" target="" class="text-decoration-none mb-2">Nosotros</a>
           </li>
           <li><a href="<?php echo $base; ?>/contacto.php" target="" rel="noopener noreferrer" title="contacto" class="text-decoration-none">
               <svg class="icon__xl">
                 <use href="<?php echo $base; ?>/assets/img/sprite.svg#letter"></use>
               </svg>
               <span class="visually-hidden">contacto</span>
             </a></li>
           <li><a href="http://www.facebook.com" target="_blank"><svg class="icon__xl">
                 <use href="<?php echo $base; ?>/assets/img/sprite.svg#rrssfacebook"></use>
               </svg><span class="visually-hidden">facebook</span></a></li>
           <li><a href="http://www.instagram.com" target="_blank"><svg class="icon__xl">
                 <use href="<?php echo $base; ?>/assets/img/sprite.svg#rrssinstagram"></use>
               </svg><span class="visually-hidden">instagram</span></a></li>
           <li><a href="https://x.com/" target="_blank" rel="noopener noreferrer" title="X" class="text-decoration-none">
               <svg class="icon__xl">
                 <use href="<?php echo $base; ?>/assets/img/sprite.svg#rrsstwitter"></use>
               </svg>
               <span class="visually-hidden">X</span>
             </a></li>
        
         </ul>
       </div>
     </div>
   </div>
   <!-- inicio seccion nav -->
   <nav class="navbar navbar-expand-lg py-0 navbar-borde">
     <div class="container-lg  align-items-center d-flex justify-content-between align-items-md-stretch">
       <a href="<?php echo $base; ?>/index.php" rel="noopener noreferrer" title="Home" class="text-decoration-none navbar-brand">
         <img src="<?php echo $base; ?>/assets/img/logolarraz.svg" alt="Inicio" class="logo">
         <span class="visually-hidden">Home</span></a>
       <button class="navbar-toggler navbar-borde" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
         <span class="navbar-toggler-icon"></span>
       </button>
       <div class="collapse navbar-collapse" id="navbarSupportedContent">
         <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
           <li class="nav-item">
             <a class="nav-link" href="<?php echo $base; ?>/pages/merceria/index.php">Mercería</a>
           </li>
           <li class="nav-item">
             <a class="nav-link" aria-current="page" href="<?php echo $base; ?>/pages/manualidades/index.php">Manualidades</a>
           </li>
           <li class="nav-item">
             <a class="nav-link" href="<?php echo $base; ?>/pages/textil/index.php">Textil</a>
           </li>
           <li>
           <li class="nav-item">
             <a class="nav-link cta" href="<?php echo $base; ?>/contacto.php">Contacto</a>
           </li>
           <li><a href="#visitanos" class="btn-cta-nav nav-item">Visitanos</a></li>
         </ul>
       </div>
     </div>
   </nav>
 </header>
 <main class=>